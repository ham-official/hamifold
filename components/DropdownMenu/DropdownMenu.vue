<template>
  <div class="relative flex" @focusout="handleFocusOut">
    <CTA v-if="!isConnected" size="lg" :color="closeCTA.color" :icon="closeCTA.icon" @click="handleConnect()" class="uppercase">{{ closeCTA.label }}</CTA>
    <CTA v-else size="lg" :color="openCTA.color" @click="dropIsOpen = !this.dropIsOpen" :iconLeft="openCTA.icon">
            {{ openCTA.label }}</CTA>
    <div v-if="dropIsOpen && isConnected" class="absolute top-16 right-0 z-10 mt-2 w-56 border border-gray-900 rounded-lg ham-shadow">
        <ul class="flex flex-col bg-white rounded-md">
          ...
          <li @click="handleDisconnect() && (dropIsOpen = !this.dropIsOpen)"  class="flex gap-2 py-2.5 px-4 hover:bg-gray-100 rounded-lg cursor-pointer">
